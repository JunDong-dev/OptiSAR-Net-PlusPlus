...
class CLIP(TextModel):
    ...
    def __init__(self, size: str, device: torch.device) -> None:
        ...
        try:
            ...
            raise ImportError("Install the CLIP dependency with `pip install -r requirements.txt`.") from error
    ...
    def tokenize(self, texts: str | list[str], truncate: bool = True) -> torch.Tensor:
        ...
        """
        return self.clip.tokenize(texts, truncate=truncate).to(self.device)
...
class MobileCLIP(TextModel):
    ...
    def __init__(self, size: str, device: torch.device, custom_weight: str = None) -> None:
        ...
        Args:
            ...
            device (torch.device): Device to load the model on.
            custom_weight (str, optional): Custom weight file path. If None, searches in order:
                1. Environment variable MOBILECLIP_WEIGHTS_DIR
                2. The repository checkpoints directory
                3. Current directory
                4. The official download URL if not found
        """
        try:
            import mobileclip
        except ImportError:
        ...
        filename = f"mobileclip_{size}.pt"

        file = None
        if custom_weight and Path(custom_weight).is_file():
            file = custom_weight
            LOGGER.info(f"Loading MobileCLIP from custom weight: {file}")
        else:
            weights_dir = os.environ.get("MOBILECLIP_WEIGHTS_DIR")
            candidate_paths = [Path.cwd() / "checkpoints" / filename, Path.cwd() / filename]
            if weights_dir:
                candidate_paths.insert(0, Path(weights_dir) / filename)
            for candidate in candidate_paths:
                if candidate.is_file():
                    file = str(candidate)
                    LOGGER.info(f"Loading MobileCLIP from: {file}")
                    break
            if file is None:
                file = filename
                LOGGER.warning(f"MobileCLIP weights not found locally, downloading to: {file}")
...
class MobileCLIP2(TextModel):
    """Apple MobileCLIP2 text encoder backed by OpenCLIP."""
    ...
    def __init__(self, size: str, device: torch.device, weight: str) -> None:
        try:
            import open_clip
        except ImportError as error:
        ...
        super().__init__()
        model_name = self.model_size_map[size]
        model_kwargs = {}
        if size not in {"s3", "s4", "l14"}:
            model_kwargs = {"image_mean": (0, 0, 0), "image_std": (1, 1, 1)}
        self.model = open_clip.create_model_and_transforms(model_name, pretrained=weight, **model_kwargs)[0]
...
def build_text_model(variant: str, device: torch.device = None) -> TextModel:
    ...
    Examples:
        ...
        >>> model = build_text_model("mobileclip2:b", device=torch.device("cuda"))  # Uses custom weights
    """
    ...
    if base == "clip":
        ...
        weights_dir = os.environ.get("MOBILECLIP_WEIGHTS_DIR")
        weight_filename = f"mobileclip2_{size}.pt"
        candidate_paths = [Path.cwd() / "checkpoints" / weight_filename, Path.cwd() / weight_filename]
        if weights_dir:
            candidate_paths.insert(0, Path(weights_dir) / weight_filename)
        custom_weight_path = None
        for candidate in candidate_paths:
            if candidate.is_file():
                custom_weight_path = str(candidate)
                break

        if custom_weight_path is None:
            locations = "\n".join(f"  - {path}" for path in candidate_paths)
            raise FileNotFoundError(
                f"MobileCLIP2 weight file '{weight_filename}' not found in:\n{locations}\n"
                "Place the file in checkpoints/ or set MOBILECLIP_WEIGHTS_DIR."
            )
        LOGGER.info(f"Loading MobileCLIP2 from: {custom_weight_path}")
        return MobileCLIP2(size, device, custom_weight_path)
    else:
        raise ValueError(
            f"Unrecognized base model: '{base}'. Supported base models: 'clip', 'mobileclip', 'mobileclip2'."
        )
