...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        if regions:
            ...
            final_labels["regions"] = regions[good]
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if "regions" in labels or "regions" in labels2:
            reg1 = labels.get("regions", np.zeros_like(labels["cls"]))
            reg2 = labels2.get("regions", np.zeros_like(labels2["cls"]))
            labels["regions"] = np.concatenate([reg1, reg2], 0)
        return labels
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]
        cls = labels["cls"]
        regions = labels.get("regions", None)
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        ...
        for j in indexes[: round(self.p * n)]:
            ...
            if "regions" in labels1 or "regions" in labels2:
                reg1 = labels1.get("regions", np.zeros_like(labels1["cls"]))
                reg2 = labels2.get("regions", np.zeros_like(labels2.get("cls", cls)))
                reg2_sel = reg2[[j]] if len(reg2) else reg2
                labels1["regions"] = np.concatenate((reg1, reg2_sel), axis=0)
...
class Format:
    ...
    def __call__(self, labels):
        ...
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        regions = labels.pop("regions", None)
        instances = labels.pop("instances")
        ...
        if regions is None:
            regions = np.zeros((nl, 1), dtype=np.float32)
        labels["regions"] = torch.from_numpy(regions).long() if nl else torch.zeros((nl, 1), dtype=torch.long)
...
class adv_RandomLoadText:
    """Sample positive, adversarial, dataset-negative, and global-negative text embeddings."""

    # Adversarial replacement rules (longer phrases matched first)
    _ADV_REPLACEMENTS = [
        # ========== Compound positional terms (longest first) ==========
        ...
        # ========== Positional phrases ==========
        ("in the middle", "in the top"),
        ("in the middle", "in the bottom"),
        ("in the middle", "in the left"),
        ("in the middle", "in the right"),
        ("in the middle", "in the corner"),
        ("in the center", "in the top"),
        ("in the center", "in the bottom"),
        ("in the center", "in the left"),
        ("in the center", "in the right"),
        ("in the center", "at the edge"),
        ("at the center", "at the edge"),
        ("at the center", "in the corner"),
        ("at the edge", "at the center"),
        ("at the edge", "in the middle"),
        ("middle of the", "top of the"),
        ("middle of the", "bottom of the"),
        ("middle of the", "left of the"),
        ("middle of the", "right of the"),
        ("center of the", "edge of the"),
        ("center of the", "corner of the"),
        ...
        ("center", "edge"),
        ("center", "corner"),
        # ========== Size adversarial pairs (adjective+noun first) ==========
        # ("a huge", "a small"),
        # ("a huge", "a tiny"),
        # ("a large", "a small"),
        # ("a large", "a tiny"),
        ...
        ("small", "large"),
        ("small", "huge"),
        # ========== Density/state adversarial pairs ==========
        ("densely packed", "sparsely distributed"),
        ...
        ("occupied", "vacant"),
        ("vacant", "occupied"),
        # ========== Visual-attribute adversarial pairs ==========
        ("bright", "dark"),
        ("dark", "bright"),
        ("clear", "blurry"),
        ("blurry", "clear"),
        ...
        ("several", "one"),
        ("numerous", "few"),
        # ========== Modality adversarial pairs ==========
        # ("sar", "optical"),
        # ("optical", "sar"),
        # ========== Other semantic adversarial pairs ==========
        ("horizontal", "vertical"),
        ("vertical", "horizontal"),
        ("wide", "narrow"),
        ("narrow", "wide"),
    ...
    def __init__(
        self,
        text_model: str,
        dataset: str = "OptSAR_RSVG",
        cache_dir: str | Path = "artifacts/text_embeddings",
        neg_text_count: int = 200,
        prompt_format: str = "{}",
        neg_samples: Tuple[int, int] = (0, 10),
        max_samples: int = 10,
        padding: bool = True,
        adv_modal_region: bool = True,
        adv_sample_ratio: float = 0.5,
    ) -> None:
        """Initialize text sampling from precomputed embeddings."""
        self.prompt_format = prompt_format
        self.neg_samples = neg_samples
        self.max_samples = max_samples
        self.padding = padding
        self.adv_modal_region = adv_modal_region
        self.adv_sample_ratio = adv_sample_ratio
        self._load_embeddings(text_model, dataset, neg_text_count, Path(cache_dir))

    def _load_embeddings(self, text_model: str, dataset: str, neg_text_count: int, cache_dir: Path) -> None:
        """Load cached text strings and embeddings."""
        neg_text_path = cache_dir / f"adv_{dataset}_global_grounding_neg_text{neg_text_count}.json"
        if not neg_text_path.is_file():
            raise FileNotFoundError(f"Negative-text cache not found: {neg_text_path}")

        with neg_text_path.open(encoding="utf-8") as f:
            self.global_grounding_neg_cats = np.array(json.load(f))

        neg_emb_path = cache_dir / text_model / f"adv_{dataset}_global_grounding_neg_embeddings_text{neg_text_count}.pt"
        train_emb_path = cache_dir / text_model / f"adv_{dataset}_train_label_embeddings.pt"
        self.global_grounding_neg_embeddings = torch.load(neg_emb_path, map_location="cpu")
        self.train_label_embeddings = torch.load(train_emb_path, map_location="cpu")
        LOGGER.info(f"Loaded adversarial text embeddings from {cache_dir}")

    def _adv_swap(self, text: str) -> List[str]:
        """Generate adversarial variants by replacing one matched phrase."""
        variants = set()
        low = text.lower()

        for src, tgt in self._ADV_REPLACEMENTS:
            if src in low:
                new_text = low.replace(src, tgt, 1)
                variants.add(new_text)

        return list(variants)

    def _sample_adversarial_texts(
        self, texts: List[str], txt_feats: List[torch.Tensor], txt_list: List[str], num_pos_samples: int
    ) -> Tuple[List[torch.Tensor], List[str]]:
        """Append valid adversarial samples up to the configured ratio and capacity."""
        if not self.adv_modal_region or len(txt_feats) >= self.max_samples:
            return txt_feats, txt_list

        adv_pool = []
        for text in texts:
            adv_pool.extend(self._adv_swap(text))

        txt_set = set(txt_list)
        valid_adv_pool = [
            adv_text for adv_text in adv_pool if adv_text in self.train_label_embeddings and adv_text not in txt_set
        ]
        random.shuffle(valid_adv_pool)

        dynamic_adv_max = int(num_pos_samples * self.adv_sample_ratio)
        max_adv_to_add = min(dynamic_adv_max, self.max_samples - len(txt_feats))
        for adv_text in valid_adv_pool[:max_adv_to_add]:
            txt_feats.append(self.train_label_embeddings[adv_text])
            txt_list.append(adv_text)
        return txt_feats, txt_list

    def _pad_features(self, txt_feats: Optional[torch.Tensor]) -> torch.Tensor:
        """Pad or truncate text features to the configured sample count."""
        if txt_feats is None or txt_feats.numel() == 0:
            if self.padding:
                indices = np.random.choice(
                    len(self.global_grounding_neg_embeddings), size=self.max_samples, replace=True
                )
                return self.global_grounding_neg_embeddings[indices]
            return torch.empty(0)

        current_len = txt_feats.shape[0]

        if current_len > self.max_samples:
            return txt_feats[: self.max_samples]

        if self.padding and current_len < self.max_samples:
            num_padding = self.max_samples - current_len
            indices = np.random.choice(len(self.global_grounding_neg_embeddings), size=num_padding, replace=True)
            pad_embeddings = self.global_grounding_neg_embeddings[indices]
            txt_feats = torch.cat([txt_feats, pad_embeddings], dim=0)

        return txt_feats

    def _sample_labels(self, cls: np.ndarray, num_classes: int) -> Tuple[List[int], List[int]]:
        """Sample positive and in-dataset negative class indices."""
        pos_labels = np.unique(cls).tolist()

        if len(pos_labels) > self.max_samples:
            pos_labels = random.sample(pos_labels, k=self.max_samples)

        max_neg = min(num_classes, self.max_samples) - len(pos_labels)
        neg_count = min(max_neg, random.randint(*self.neg_samples))

        neg_labels = [i for i in range(num_classes) if i not in pos_labels]
        neg_labels = random.sample(neg_labels, k=neg_count) if neg_count > 0 else []

        return pos_labels, neg_labels

    def _update_instances(self, labels: dict, cls: np.ndarray, label2ids: Dict[int, int]) -> np.ndarray:
        """Retain positive instances and remap their class indices."""
        ...
        return np.array(new_cls)

    def __call__(self, labels: dict) -> dict:
        """Apply positive, background, adversarial, dataset-negative, then global-negative sampling."""
        assert "texts" in labels, "Text labels are required."

        class_texts = labels["texts"]
        num_classes = len(class_texts)
        cls = np.asarray(labels.pop("cls"), dtype=int)

        pos_labels, neg_labels = self._sample_labels(cls, num_classes)
        ...
        for label in pos_labels:
            ...
            prompt = self.prompt_format.format(random.choice(prompts))
            pos_texts.append(prompt)
            if prompt in self.train_label_embeddings:
                txt_feats.append(self.train_label_embeddings[prompt])
                txt_list.append(prompt)

        if "background" in self.train_label_embeddings and "background" not in txt_list:
            txt_feats.append(self.train_label_embeddings["background"])
            txt_list.append("background")

        txt_feats, txt_list = self._sample_adversarial_texts(pos_texts, txt_feats, txt_list, len(pos_labels))

        for label in neg_labels:
            if len(txt_feats) >= self.max_samples:
                break
            prompts = class_texts[label]
            prompt = self.prompt_format.format(random.choice(prompts))
            if prompt in self.train_label_embeddings and prompt not in txt_list:
                txt_feats.append(self.train_label_embeddings[prompt])
                txt_list.append(prompt)

        label2ids = {label: i for i, label in enumerate(pos_labels)}
        labels["cls"] = self._update_instances(labels, cls, label2ids)
        ...
        txt_feats = self._pad_features(txt_feats)
        if self.padding:
            assert txt_feats.shape[0] == self.max_samples, (
                f"txt_feats={txt_feats.shape[0]} vs max_samples={self.max_samples}"
            )
